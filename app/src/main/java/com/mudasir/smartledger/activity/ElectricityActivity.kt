...
import com.mudasir.smartledger.adapter.ElectricityAdapter
...
class ElectricityActivity : AppCompatActivity(), NavigationView.OnNavigationItemSelectedListener {
    ...
    private fun setupUI() {
        ...
        com.mudasir.smartledger.data.DataCache.cachedElectricity?.let { cached ->
    ...
    private fun loadRecords(sortType: SortType) {
        ...
        loadJob = lifecycleScope.launch {
            ...
            flow.collect { list ->
                com.mudasir.smartledger.data.DataCache.cachedElectricity = list
                val rv = findViewById<RecyclerView>(R.id.rvElectricity)
    ...
    private fun showDeleteConfirmationDialog(items: List<Electricity>, mode: ActionMode) {
        val dialog = DialogHelper.createConfirmationDialog(this) { views ->
            ...
            if (items.size == 1) {
                ...
                views.detailTitle.text = "${item.totalUnits ?: 0} Units"
    ...
    override fun onResume() {
        ...
        navView.setCheckedItem(R.id.nav_electricity)
    }
}